...
public class QihooAPPLoaderTest {
	...
	public void getAppList(){
		...
		try {
			...
			userInfo.setLat("42.2971123203");
			userInfo.setLon("118.930761192");
			...
			//app.setPkgName("com.alphago.appmarket");
			//app.setAppName("多彩应用");
			
			
			app.setPkgName("com.main.zen");
			app.setAppName("TX主题商店");
			app.setVersionCode("13");
			app.setVersionName("2.0.27");
			adreq.setAppInfo(app);
			
			InterParam interParam=new InterParam();
			interParam.setKeyWord("网易新闻");
			interParam.setPkgName("com.sohu.newsclient");
			adreq.setInterParam(interParam);
			
            adreq.setAppSpaceInfo(space);
            adreq.setJoinSource(JoinSource.QIHOO_APPSTORE);
           ...
           // adreq.setInterType(InterType.APP_RECOMMEND_SEARCH);
            ...
            from.setKey("from");
            //from.setValue("moyu");
            from.setValue("zhangkutxshop");
            
            extL.add(from);
            ExtData ch=new ExtData();
            ch.setKey("ch");
            //ch.setValue("300222");
            ch.setValue("300266");
            extL.add(ch);
            ExtData adspaceid=new ExtData();
            adspaceid.setKey("adspaceid");
            //adspaceid.setValue("aPPmupEK62");
            adspaceid.setValue("k5uH7sBCjK");
