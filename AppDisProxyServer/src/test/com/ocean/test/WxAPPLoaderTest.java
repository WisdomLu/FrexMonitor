...
public class WxAPPLoaderTest {
	...
	public void getAppList(){
		...
		try {
			...
			interParam.setKeyWord("旅游");
			adreq.setInterParam(interParam);
			
            adreq.setAppSpaceInfo(space);
            adreq.setJoinSource(JoinSource.WANGXIANG_APPSTORE);
            ...
            from.setKey("channel_id");
            from.setValue("467b31eb4f");
            extL.add(from);
            ExtData token=new ExtData();
            token.setKey("key");
            token.setValue("c139d3cdd33b75abdc6d59792b88b8c1");
            extL.add(token);
            ExtData iv=new ExtData();
            iv.setKey("sign");
            iv.setValue("WangXiang@channel#2018%cpdengine!");
            extL.add(iv);
