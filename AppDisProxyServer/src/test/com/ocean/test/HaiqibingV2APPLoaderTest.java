...
public class HaiqibingV2APPLoaderTest {
	...
	public void getAppList(){
		...
		try {
			...
			List<ExtData> interEL=new ArrayList<ExtData>();
			ExtData isPackageShild=new ExtData();
			isPackageShild.setKey("isPackageShild");
			isPackageShild.setValue("1");
			interEL.add(isPackageShild);
            interParam.setRequiredParam(interEL);
			
			adreq.setInterParam(interParam);
			
            adreq.setAppSpaceInfo(space);
            adreq.setJoinSource(JoinSource.HAIQIBING_APPSTORE);
            ...
            ExtData channel=new ExtData();
            channel.setKey("channel");
            channel.setValue("haiqibin");
            extL.add(channel);  
