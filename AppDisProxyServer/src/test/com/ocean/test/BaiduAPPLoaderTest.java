...
public class BaiduAPPLoaderTest {
	...
	public void getAppList(){
		...
		try {
			...
			interParam.setKeyWord("搜书");
			adreq.setInterParam(interParam);
			
            adreq.setAppSpaceInfo(space);
            adreq.setJoinSource(JoinSource.BAIDU_APPSEARCH);
            ...
            from.setKey("from");
            from.setValue("1019356t");
            extL.add(from);
            ExtData token=new ExtData();
            token.setKey("token");
            token.setValue("ivviyfbx");
            extL.add(token);
            ExtData iv=new ExtData();
            iv.setKey("iv");
            iv.setValue("ef0f900868268329");
            extL.add(iv);
            
            ExtData key=new ExtData();
            key.setKey("key");
            key.setValue("f4266ade404b1bb047132bd7");
            extL.add(key);
