...
import com.ocean.persist.app.dis.appSearch360T2.appRcmdSearch.AppRcmd360T2Puller;
...
@Component(value="Qihoo360T2AppRcmdSearchHandler")
public class Qihoo360T2AppRcmdSearchHandler  extends QihooBaseHandler{

	@Autowired
	private AppRcmd360T2Puller puller;

	@Override
	public AppDisResponse invok(AppDisRecomReq req) throws AppDisException {
		// TODO Auto-generated method stub
		AppRcmd360T2Request request=parseKeyWordSearchParam(req);
		//return puller.api(request,req.getUserInfo().getUid(),String.valueOf(req.getAppSpaceInfo().getAdSpaceId()));
		return (AppRcmd360T2Response)this.invoke(request, AppRcmd360T2Response.class, req.getUserInfo().getUid(),String.valueOf(req.getAppSpaceInfo().getAdSpaceId()));
		
		
	}
	private AppRcmd360T2Request parseKeyWordSearchParam(AppDisRecomReq req) throws AppDisException{
		AppRcmd360T2Request request=new AppRcmd360T2Request();
		...
		if(StringUtil.isEmpty(interParam.getKeyWord())&&StringUtils.isEmpty(interParam.getHotWord())){
			throw new AppDisException(AppDisErrorCode.RC_PARAM_ERROR,"app recommend search parameter(keyword) is empty!");
		}
		String keyWord=interParam.getKeyWord();
		if(StringUtil.isEmpty(keyWord)){
			keyWord=interParam.getHotWord();
		}
		request.setRecom_appname(keyWord);
		if(StringUtil.isNotEmpty(interParam.getPkgName())){
			request.setRecom_apppkg(interParam.getPkgName());
		}
	...
	public AppRcmd360T2Puller getPuller() {
		return puller;
	}
	public void setPuller(AppRcmd360T2Puller puller) {
		this.puller = puller;
	}
	@Override
	public AsynAbstractTask packageTask(Parameter params, String hashCode) {
		// TODO Auto-generated method stub
		QihooType2RecmdTask task = new QihooType2RecmdTask();
		AppRcmd360T2Request param = (AppRcmd360T2Request) params;
