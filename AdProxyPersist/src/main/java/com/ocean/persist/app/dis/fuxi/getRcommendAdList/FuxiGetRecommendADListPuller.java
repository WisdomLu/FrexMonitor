package com.ocean.persist.app.dis.fuxi.getRcommendAdList;
...
import com.ocean.persist.app.dis.fuxi.FuxiAbstractQQDownloaderPuller;
...
/** * @author Alex & E-mail:569246607@qq.com
      ...
      @version 1.0 
 */
@Component(value="FuxiGetRecommendADListPuller")
public class FuxiGetRecommendADListPuller    extends FuxiAbstractQQDownloaderPuller{
	...
	public AppDisResponse api(AdDisParams params,String ... exts)
		...
		logger.info("joinAppStore:HAIQIBING_APPSTORE  getRecommendADList {} request info:{}",exts,requestBody);
		GetRecommendADListResponse data;
		try {
			String url=getUrl((QQDownloaderRequest)params);
			logger.info("joinAppStore:HAIQIBING_APPSTORE  getRecommendADList {} url:{}",exts,url);
			String result = HttpClient.getInstance().post(url, requestBody.toString(),headers);
			logger.info("joinAppStore:HAIQIBING_APPSTORE getRecommendADList {}  ad return result:{}",exts,result);
			...
			logger.info("joinAppStore:HAIQIBING_APPSTORE  getRecommendADList {} pull api  time cost:{} ms",exts,System.currentTimeMillis() - ts);
