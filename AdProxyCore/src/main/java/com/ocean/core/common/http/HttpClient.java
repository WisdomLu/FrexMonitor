...
public class HttpClient {
	...
	private   HttpClient(){
		try {
			...
			// 最大连接数设置
			int maxTotal=SystemContext.getDynamicPropertyHandler().getInt(HTTP_MAX_CONNECTION, 1000);
			connectionPoolMgr.setMaxTotal(maxTotal);
			int perRoute =SystemContext.getDynamicPropertyHandler().getInt(HTTP_PER_ROUTE_CONNECTION, 200);//到每个路由/主机的最大连接数
			...
			int timeOut=SystemContext.getDynamicPropertyHandler().getInt(HTTP_TIME_OUT, 1000);
...
//		return null;
	...
	public String postBytesZip(String url, byte[] body, 
			Map<String, String> headers) throws HttpInvokeException{
		...
		try {
			...
			return httpResp.getEntity()==null?null:unzip(httpResp.getEntity());
		} catch (ClientProtocolException e) {
			logger.error("http request error(ClientProtocolException),url:{},error msg:{}",url,e.getMessage(),e);
			throw new HttpInvokeException(ErrorCode.INTER_ERROR, e.getMessage());
		} catch (IOException e) {
	...
	private String unzip(HttpEntity entity){
        try {
        	if(entity.getContentLength()<0){
        		return null;
        	}
        	
        	logger.info("content encoding:{},lenth:{},type:{}",entity.getContentEncoding()==null?"unknow":entity.getContentEncoding().getValue(),entity.getContentLength(),entity.getContentType());
		...
		try {
			...
			if (HttpStatus.SC_OK != status) {
				...
				try{
					...
					result="try to parse response but failed";
				}
				
				
				throw new HttpInvokeException(status,"http request failed url:"+method.getURI()+",return unexpect status:"+status +" and result:"+result);
			}
			//logger.info("content encoding:{},lenth:{},type:{}",httpResp.getEntity().getContentEncoding()==null?"unknow":httpResp.getEntity().getContentEncoding().getValue(),httpResp.getEntity().getContentLength(),httpResp.getEntity().getContentType());
